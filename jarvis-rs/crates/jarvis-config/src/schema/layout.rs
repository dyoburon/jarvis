...
mod tests {
    ...
    fn layout_config_defaults() {
        ...
        assert!((config.border_width - 0.0).abs() < f64::EPSILON);
        assert_eq!(config.outer_padding, 0);
        assert!((config.inactive_opacity - 1.0).abs() < f64::EPSILON);
    }
